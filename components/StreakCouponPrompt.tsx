...
import { computeStreak, COUPON_MILESTONES, type StreakRec } from '@/lib/streak'
...
export default function StreakCouponPrompt({ studentId, chronoRecs }: { studentId: number; chronoRecs: StreakRec[] }) {
  ...
  async function claim() {
    if (!prompt) return
    setBusy(true)
    await supabase.from('student_coupons').insert({ student_id: studentId, milestone: prompt.milestone, streak_value: prompt.streakValue })
