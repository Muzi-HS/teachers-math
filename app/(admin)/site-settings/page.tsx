...
export default function SiteSettingsPage() {
  ...
  if (loading) return (
    <div style={{ padding: mobileMode ? '16px 14px 88px' : '28px 32px' }}>
      <p style={{ fontSize: 13, color: tx3 }}>불러오는 중...</p>
    </div>
  )

  return (
    <div style={{ padding: mobileMode ? '16px 14px 88px' : '28px 32px', fontFamily: "'Noto Sans KR',sans-serif", maxWidth: 640 }}>
      <div style={{ marginBottom: mobileMode ? 14 : 20 }}>
        <h1 style={{ fontSize: mobileMode ? 17 : 21, fontWeight: 700, color: tx }}>사이트 설정</h1>
        <p style={{ fontSize: 13, color: tx2, marginTop: 4 }}>홈페이지에 표시되는 계절 시각 효과를 관리합니다</p>
      </div>
